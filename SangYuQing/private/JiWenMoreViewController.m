...
- (void)viewDidLoad {
    ...
    _tableview->_tableView.estimatedSectionFooterHeight = 0;
     _tableview.refreshStyle = HZRefreshTableStyleHeader;
...
-(void)requestJW{
    ...
    [client hcGET:@"/v1/gongmu/articles" parameters:@{@"id":_sz_id}  success:^(NSURLSessionDataTask *task, id object) {
        if ([object[@"state_code"] isEqualToString:@"0000"]) {
            NSArray *list = [MTLJSONAdapter modelsOfClass:[ArticleModel class] fromJSONArray:object[@"data"][@"CemeteryInfo"][@"articles"] error:nil];
...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
    ...
    UITableViewRowAction *action = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"删除" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        ArticleModel*model = _list[indexPath.row];
        HZHttpClient *client = [HZHttpClient httpClient];
        [client hcGET:@"/v1/jiwen/del" parameters:@{@"id":[NSString stringWithFormat:@"%zd",model.szjw_id]} success:^(NSURLSessionDataTask *task, id object) {
            if ([object[@"state_code"] isEqualToString:@"0000"]) {
                [self headerRefresh];
